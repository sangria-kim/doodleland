...
- 내부 빈 공간과 질감 보존을 위한 후속 복원 전략 실험
- placeholder 파일(`image_processor.dart`, `stage_painter.dart`, `background_selector.dart`) 정리
- 둥실 둥실을 나뭇잎 떨어지는 것처럼 위에서 아래로 이동하고 아래로 다 이동하면 다시 위에서 나타나도록
- 기차처럼 왼쪽에서 오른쪽 끝까지 이동 후 다시 왼쪽에서 나타나도록 하는 모션 
- 그림 고르기 화면에서 그림 반전, 회전 할 수 있도록 기능 추가
- 그림 드래그하면 좌->우 이동으로 초기화하는 것이 아니라 현재 움직이던 방향으로 다시 움직이도록 수정
---
