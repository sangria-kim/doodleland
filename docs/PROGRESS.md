...
**다음 예정 작업**
- 실제 기기(저사양/고사양) 기준 BGM/SFX/홈 진입 음성 볼륨 밸런싱 및 기기별 청감 점검
...
#### feat: add pororo playground stage bgm
- 뽀로로 놀이터 배경에 전용 BGM을 연결하고 무대 배경 선택 시 해당 트랙이 재생되도록 확장했습니다.
- 무대 전환 중 홈 진입 음성이 함께 시작되지 않도록 오디오 조건과 취소 흐름을 정리했습니다.
- 기능 반영으로 버전을 `1.19.0+11900`으로 상향했습니다.

#### refactor: share menu action button widget
- 홈 화면과 그림 만들기 진입 화면의 버튼을 공통 컴포넌트로 통합해 중복 구현을 제거했습니다.
- 민트 톤 버튼, 투명도, 간격, 1x3 비율 배치 값을 공통 파라미터로 정리해 유지보수성을 개선했습니다.
- 기존 동작 흐름(그림 만들기, 내 그림, 놀이 시작, 캡처 진입)은 유지하며 UI 일관성 개선을 완료했습니다.

#### ui: refine home and capture button experience
- 홈 화면 버튼의 배치/간격/크기를 1x3 반응형 구조로 정리해 화면 밀도별 일관성을 높였습니다.
- `그림 만들기`/`내 그림`/`놀이 시작` 텍스트와 캡처 진입 버튼을 동일한 민트 계열 및 YoonChildfundkoreaMinGuk 폰트로 통일했습니다.
- 캡처 폰트 자산 매핑 문제를 수정해 앱 시작 시 번들 에셋 빌드 오류를 제거하고 테스트 흐름을 안정화했습니다.

### 2026-04-17

#### ui: update entry screens with fullscreen illustrated background
- 그림 만들기, 놀이 시작, 내 그림 화면에 공통 배경 이미지를 풀스크린으로 적용했습니다.
- 상단 안전영역과 뒤로가기 오버레이까지 동일한 배경으로 덮어 메인 화면과 진입 경험을 맞췄습니다.
- 사용자 체감 UI 변경 기준으로 버전을 `1.18.0+11800`으로 상향했습니다.

#### feat: add alternating home entry voice playback
- 메인 화면 진입 시 녹음 음성 2개를 번갈아 재생하도록 홈 오디오 흐름을 확장했습니다.
- 버튼 효과음과 홈 진입 음성이 서로 끊기지 않도록 전용 재생 채널과 진입/복귀 테스트를 함께 보강했습니다.
- 기능 반영으로 버전을 `1.17.0+11700`으로 상향했습니다.

#### ui: update back button assets and overlay placement
- 공통 뒤로가기 에셋 버튼을 추가하고 무대 화면을 제외한 주요 화면의 뒤로가기 노출 방식을 동일한 오버레이 위치 기준으로 정리했습니다.
- 무대 화면은 기존 Material 뒤로가기 버튼을 유지하고, 다른 화면은 AppBar 의존을 제거해 위치와 크기를 일관되게 맞췄습니다.
- UI 미세 조정 기준으로 버전을 `1.16.1+11601`로 상향했습니다.

### 2026-04-16

#### feat: add picture library management and unified picker cards
- 홈에 `내 그림` 진입과 `/library` 관리 화면을 추가해 저장 그림 조회·삭제 흐름을 완성했습니다.
- 내 그림과 그림 고르기 화면의 카드 디자인을 4열 썸네일 중심 구조로 통일하고 관련 문서와 테스트를 함께 갱신했습니다.
- 기능 반영으로 버전을 `1.16.0+11600`으로 상향했습니다.

#### feat: add passthrough save for preprocessed transparent inputs
- 이미 투명한 입력 이미지는 저장 시 추가 배경 제거를 건너뛰고 그대로 투명 산출물로 저장하도록 확장했습니다.
- 저장 메타데이터를 최종 transparent 산출물 기준으로 정리하고 관련 문서와 저장 유스케이스 테스트를 함께 보강했습니다.
- 기능 반영으로 버전을 `1.15.0+11500`으로 상향했습니다.

#### ui: update home background to full-screen composition
- 홈 메인 배경이 상단 안전영역까지 채워지도록 전체 레이어 레이아웃을 조정했습니다.
- 메인 배경/타이틀 이미지 보정본을 함께 반영해 전체 모드에서 시각 정합성을 개선했습니다.
- UI 미세 조정 기준으로 버전을 `1.14.1+11401`로 상향했습니다.

#### ui: animate layered home background transitions
- 홈 메인 화면에 배경 4레이어(기본/전경/차량/타이틀)를 적용하고 시간차 등장 애니메이션을 추가했습니다.
- 다른 화면에서 홈으로 복귀했을 때도 애니메이션이 다시 재생되도록 라우트 복귀 트리거를 연결했습니다.
- 버튼 오버레이 위치와 투명도를 조정하고 버전을 `1.14.0+11400`으로 상향했습니다.
...
- 나풀나풀(Fluttering) 모션을 새롭게 추가해 위에서 천천히 낙하하고 좌우로 흔들리는 동작을 구현했습니다.
- 낙하 시작/종료 경계를 확장해 화면 하단 완전 이탈 후 재생성되며, 하단 통과 시 부드러운 페이드 아웃을 적용했습니다.
- 무대와 미리보기에서 동일한 흔들림 패턴으로 동작하도록 동기화하고 `versionName`을 `1.13.0`, `versionCode`를 `11300`으로 상향합니다.

#### ui: rebalance motion step layout
- 그림 미리보기와 움직임 고르기 영역을 동일 비율로 정렬해 화면 레이아웃 균형을 맞췄습니다.
- 모션 버튼을 2열 2행 카드로 고정하고 스크롤을 제거해 작은 화면에서도 한 번에 확인 가능한 상태로 조정했습니다.
- UI 변경으로 `versionName`을 `1.12.1`, `versionCode`를 `11201`로 상향합니다.
...
- 기능 반영으로 `versionName`을 `1.11.0`, `versionCode`를 `11100`으로 상향합니다.
