...
| 프리셋 | 라벨 | 현재 구현 |
|--------|------|-----------|
| `floating` | 둥실둥실 | 2초 주기 상하 사인 이동 |
| `bouncing` | 통통 점프 | 1.2초 주기 수직 바운스 |
| `gliding` | 씽씽 활공 | 3초 주기 좌우 왕복 |
| `rolling` | 데굴데굴 | 2.5초 주기 좌우 이동 + 회전 |
| `spinning` | 빙글빙글 | 1.5초 주기 제자리 회전 |

기본 구현 방식:
- 캐릭터별 `AnimationController`
- 무한 반복 재생
- 드래그 중 일시 정지, 종료 후 재개
