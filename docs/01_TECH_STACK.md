...
- 처리 순서:
  1. 저장 직전에 입력 이미지의 `nonOpaquePixelRatio(alpha < 255 비율)`를 계산
  2. `nonOpaquePixelRatio >= 5%`이면 입력 이미지를 그대로 최종 투명 PNG로 저장
  3. 미만이면 입력 이미지를 읽고 최대 `1500px`로 축소
  4. 가장자리 샘플링으로 배경색 추정
  5. 색상 거리 threshold로 전경 후보 분리
  6. 가장자리 flood fill로 배경 누수 제거
  7. connected component 기반 노이즈 제거
  8. 투명 PNG 저장
- 품질 경고는 remover 경로의 `transparentAreaRatio(alpha == 0 비율)`가 `5% 미만` 또는 `95% 초과`일 때 반환합니다.
- 이미 투명한 입력이라도 투명 영역이 매우 작으면 remover 경로로 처리될 수 있습니다.
- 현재는 미리보기 진입 시점이 아니라 저장 버튼을 눌렀을 때 최종 투명 산출물이 생성됩니다.
