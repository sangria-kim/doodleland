...
- 단, `docs/` 하위 문서와 `AGENTS.md`, `CLAUDE.md` 등 운영 문서 작업은 별도 `feature` 브랜치 없이 `main`에서 바로 수정하고 커밋/푸시할 수 있다.

### [1-1] 작업 시작 전 Branch Out 규칙

작업 시작 절차:
1. 현재 브랜치 확인: `git branch --show-current`
2. `main`인 경우 → 문서 작업이 아니면 작업 중단, feature 브랜치 생성 후 진행
   ```bash
   git checkout -b feature/<description> main
   ```
3. 문서 작업(`docs/`, `AGENTS.md`, `CLAUDE.md` 등)인 경우에는 `main`에서 바로 작업할 수 있다.
4. 그 외 작업은 생성한 브랜치에서 이후 모든 작업을 진행한다.
...
문서 작업(`docs/`, `AGENTS.md`, `CLAUDE.md` 등) 시작 전 반드시 확인:
- [ ] 변경 범위가 문서에만 한정되는가?
- [ ] `main`에서 직접 작업해도 되는 문서 작업이 맞는가?
- [ ] 커밋 메시지가 기존 컨벤션(`docs:` 등 subject + body 구조)을 따르는가?

**`main` 브랜치 감지 시 동작:**
1. 문서 작업이 아니면 작업 중단
2. "현재 main 브랜치입니다. feature 브랜치 생성이 필요합니다" 안내
3. 브랜치 생성 명령 제안 후 대기
4. 문서 작업이면 `main`에서 진행하되, 기존 커밋 컨벤션을 그대로 적용
